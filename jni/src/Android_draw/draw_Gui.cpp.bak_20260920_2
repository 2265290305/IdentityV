    ...
    for (int i = 0; i < 数量; i++){
        ...
        if (W>0){
		    ...
		    if (show_draw_Prop&&data[i].阵营==4){
                // 自己身上的道具坐标跟自身基本重合，按距离剔除。
                // 注意全局的 距离 是 int(见文件头 `int 距离;`)，不到1米会被截断成0，
                // 所以这里必须用浮点重算一次，否则 <0.5 这个阈值根本表达不出来。
                float 道具距离 = sqrt(pow(D.X - Z.X, 2) + pow(D.Y - Z.Y, 2) + pow(D.Z - Z.Z, 2)) / 距离比例;
                if (道具距离 >= 0.5f) {
