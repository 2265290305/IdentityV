...
// ---- 场景对象"是否本局真实存在"判定 ----
// 游戏把所有候选刷新点都实例化成对象放进数组，本局只激活其中一部分。
// 未激活的那些类名、坐标全都正常，光看类名/坐标区分不出来。
//
// 判据是 +0x30：真正进入场景后才会挂上的节点指针，未实例化的对象这里恒为 NULL。
// 实测一局(军工厂)：密码机 16个对象→7个非空(=真实台数)；椅子 24→7；玩家角色 8→5(=实际人数)。
//
// 注意 +0x1a0 是"类型"字段(450=玩家角色 500=可交互物)，**不是存在性**。
// 曾经误用它做判据，结果一个未实例化的密码机 0x77D2330220 的 +0x1a0 恰好就是 500，
// 照样被画出来。类型字段必须和 +0x30 一起用。
static inline bool 实体已实例化(uintptr_t obj)
{
    uint64_t p = getPtr64(obj + 0x30);
    return p > 0x5000000000 && p < 0x8000000000;
}
static inline bool 是真实密码机(uintptr_t obj)
{
    // +0x30 排掉未实例化的；+0x1a0==500 排掉一个位于原点(0,0,0)的占位对象(它 +0x30 非空但类型不对)
    // 备用判据(同样实测能精确给出7台)：+0x1e0==1 且 +0x240==2
    return 实体已实例化(obj) && getFloat(obj + 0x1a0) == 500.0f;
}
...
void Draw_Main(ImDrawList *Draw){
    ...
    for (int i = 0; i < 数量; i++){
        ...
        if (W>0){
            if (Debugging){
                // 调试绘制是独立分支，不走上面任何类别过滤，所以会把未实例化的池子对象
                // 一起画出来(表现为"同一个位置两份不同地址")。这里统一挡掉。
                // 想看全部对象(比如排查新偏移时)，把下面这行注释掉即可。
                if (!实体已实例化(data[i].obj)) continue;
                std::string test;
